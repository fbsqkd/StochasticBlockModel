...
for case in range(0,num_sim):
    T = int(n*(n-1)/2)
    p_in = (2.0+2.0*case)* np.log(n)/n
    p_out = 1.0* np.log(n)/n

    for sim in range(0,sim_time):
        #Create Adjacency Matrices with Labels
        AmatrixP,AmatrixN = Ls.genAmatrixT(n=n, p_in = p_in, p_out = p_out,T = T)
        ...
        errors[case,2,:] = np.exp(-(T/n)*Ls.DLP(p=p_in,q=p_out))

        #Create Adjacency Matrices with Labels for Adaptive
        AmatrixP,AmatrixN = Ls.genAmatrixT(n=n, p_in = p_in, p_out = p_out ,T = int(0.45*T))
        labels = Ls.SpectralCluster(matrix= AmatrixP , niter = 100, n=n, K=K )
        ...
        AmatrixP2,AmatrixN2 = Ls.genAmatrix2T(indices1=indices1,indices2=indices2, p_in = p_in, p_out = p_out,T = int(0.45*T))
        ...
        AmatrixP2,AmatrixN2 = Ls.genAmatrix3T(n=n, indices1=indices1,indices2=indices2,indices3=indices3, p_in = p_in, p_out = p_out,T = int(0.1*T))
